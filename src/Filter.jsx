...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		var row = allRows.find(v => v.name == data[0].split('___')[1]);
		if (row) {
			let r = Math.round(row.qo1) == Math.round(row.tmax) && (row.pd1 == 0 || row.po1 > row.tmax) ? true : false;
			return r;
		} else return false;
	},
});
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		var row = allRows.find(v => v.name == data[0].split('___')[1]);
		if (row) {
			let r = Math.round(row.qo1) == Math.round(row.tmax) && (row.pd1 == 0 || row.po1 > row.tmax) ? false : true;
			return r;
		} else return false;
	},
});
...
filters.push({
	...
	func: (settings, data, dataIndex, obj) => {
		var row = allRows.find(v => v.name == data[0].split('___')[1]);
		if (row) {
			let r = Math.round(row.po1) == Math.round(row.tmin) && (row.qd1 == 0 || row.qo1 < row.tmin) ? true : false;
			return r;
		} else return false;
	},
});
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		var row = allRows.find(v => v.name == data[0].split('___')[1]);
		if (row) {
			let r = Math.round(row.po1) == Math.round(row.tmin) && (row.qd1 == 0 || row.qo1 < row.tmin) ? false : true;
			return r;
		} else return false;
	},
});
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		...
		if (v && v.hist) {
			...
			let sum = 0;
			let avg = v.hist.filter((v, i) => i > len - 1 - dargir).forEach(v => (sum += v.pl));
			avg = sum / dargir;
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		...
		if (v.Sell_I_Volume && v.Sell_N_Volume) {
			...
			if (
				(Number(v.Sell_N_Volume) / (Number(v.Sell_I_Volume) + Number(v.Sell_N_Volume))) * 100 >
				forushHoghughi
			) {
				return true;
			} else {
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		...
		if (v.Buy_I_Volume && v.Buy_N_Volume) {
			if ((Number(v.Buy_N_Volume) / (Number(v.Buy_I_Volume) + Number(v.Buy_N_Volume))) * 100 > kharidHoghughi) {
				return true;
			} else {
				return false;
			}
...
filters.push({
	...
	func: (settings, data, dataIndex) => {
		...
		if (v.Buy_I_Volume && v.Buy_N_Volume) {
			...
			if (b1 > 0 && (b1 / (Number(v.Buy_I_Volume) + Number(v.Buy_N_Volume))) * 100 > kharidKhalesHoghughi) {
				return true;
			} else {
				return false;
			}
...
class Filter extends React.Component {
	...
	KharidKhalesHoghughi = e => {
		kharidKhalesHoghughi = e.target.value;
		if (kharidKhalesHoghughi != 0) {
		...
		return (
			<div style={{margin: '5px'}}>
				...
				<FormControlLabel
					...
					label={
						<Typography style={{fontFamily: 'Courier New, Courier, monospace'}}>نمایش دیده بان</Typography>
					}
				/>
				...
				/>
				<div style={{float: 'right'}}>
					<span style={{fontFamily: 'Courier New, Courier, monospace'}}>{'حجم بیشتر از'}</span>
					<TextField
						select
						id="filled-select-currency"
						inputProps={{
					...
					<span style={{fontFamily: 'Courier New, Courier, monospace'}}>{'برابر  میانگن ماه'}</span>
				</div>

				<p style={{clear: 'right'}} />
				<div style={{float: 'right'}}>
					<span style={{fontFamily: 'Courier New, Courier, monospace'}}>{'قیمت پایانی مساوی میانگین'}</span>
					<TextField
						select
						id="filled-select-currency"
						inputProps={{
					...
					<span style={{fontFamily: 'Courier New, Courier, monospace'}}>{'روز قبل'}</span>
				</div>
