							...
							.map(
								(v, i) => (
									...
									(
										<tr key={v.l18 + i}>
											{
												(portfo && portfo.length > 1 && portfo.find(v1 => v1.symbol == v.name)
											...
											{
												((index = data.findIndex(v1 => v1.l18 && v1.l18 == v.l18)),
												(t1 =
													numeral(v.tvol)
														.format('0a')
														.toUpperCase()
											...
											<td data-sort={v.Buy_N_Volume ? v.Buy_N_Volume : 0} style={{padding: 0}}>
												{v.ct
											...
											{
												<td data-sort={v.Buy_CountN ? v.Buy_CountN : 0} style={{padding: 0}}>
													{v.ct
											...
											<td>
												<a
													href={'https://www.sahamyab.com/hashtag/' + v.name + '/post'}
													target="_blank">
													<img
														style={{
															display: 'block',
...
function Table(props) {
	...
	let Plot = (canvasId, data, min, max, op) => {
	...
	let PlotVal = (canvasId, data, min, max, color) => {
		let canvas = document.getElementById(canvasId);
		let ctx = canvas.getContext('2d');
	...
	$('table> tbody> tr> td> div>a').on('mousedown', function(event) {
		...
		localStorage.setItem('iterateListIndex', JSON.stringify(iterateListIndex));
	});
	//}

	//componentWillUnmount() {}

	let ChangeSettings = obj => {
		state.settings(obj);
	};
...
function GetMM(v) {
	if (v.hist) {
		...
		v.pc <= max ? (v.mmY = numeral(-((max - v.pc) / max) * 100).format()) : (v.mmY = 0);
	} else {
		v.mm = 0;
		v.mmY = 0;
	}
